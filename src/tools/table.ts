    ...
    inputSchema: {
      ...
      properties: {
        ...
        projection: {
          type: 'string',
          description:
            'Projection path, relative to the repository root unless absolute; required for markdown, responsibility, briefs and xlsx',
        },
        project: {
          type: 'string',
  ...
  switch (verb) {
    ...
    case 'export': {
      ...
      if (kind === 'responsibility') {
        if (projection === undefined) {
          return refuse(
            `${name} responsibility needs \`projection\`: the view whose subjects are the rows.`,
          )
        }
        return answer(exportResponsibility(workspace, projection), ({ markdown }) => markdown)
      }
      if (kind === 'briefs') {
        if (projection === undefined) {
          return refuse(
