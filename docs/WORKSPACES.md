...
yarramate context projections/current.yaml yarramate.workspace.yaml
yarramate view projections/current.yaml yarramate.workspace.yaml
```

`check` validates declared projection structures and adapter mappings in
addition to compiling profiles and native documents. It also evaluates
declared evidence against the resulting graph. `compile`, `context`, and
`view` compile only profiles and native documents because projections,
adapter mappings, and evidence do not become semantic claims. Projection files
remain explicit command arguments; the manifest catalogue makes them
discoverable without selecting a default.
...
`yarramate init <directory>` creates both `architecture/main.yaml` and
