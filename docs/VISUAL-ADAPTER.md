...
- `showResponsibility` — draws the `responsible`, `consulted` and `informed` edges (`yarramate/policy@0.2`, ADR 0159), labelled "is responsible for", "is consulted on", "is informed of". Off when absent: a role responsible for eight applications is eight lines out of one box, and the subject's properties read the letters either way, as "Responsible", "Consulted" and "Informed" rows under Owner, and on a person under the same three labels with the preposition in the value, "for Portal, API". A toggle sits beside the badge toggles and writes into the view like them.
- Readings the endpoints decide (ADR 0160) — an `influence` from a `yarramate/policy@0.3` risk reads "threatens", one into a risk reads "mitigates", an `association` from an assumption reads "bears on"; resolved through the endpoint lineages in the projection and carried on the edge as `reading`, which the label speaks before any kind reading. Every other edge reads as its kind does.
...
When the workspace carries its own catalogues (ADR 0129) the overlay also lists every catalogue in the composed set as `catalogues`, and the section's footer names them all (#530); a single catalogue reads as it always did.

Every fit the canvas performs, the first placement included, keeps the graph below the band the canvas toolbar overlays (#533): the app measures the controls' lower edge and the canvas fits into the height that is left, so the top row of a view is never under the filter and the layout selects on first paint.

Since ADR 0154 (#534) the overlay also names the **next question**: `interrogation.next` is the row `design` would serve first (the first open question in wave order, then catalogue order; a subject-scoped question with its first open subject's id and name), computed beside the lists by the same walk. The app draws it three ways with no mount option: the bottom panel's **Next question** tab, first in the strip and the default, whose status line says the question while the panel stays shut and which, opened, offers the way there (select the subject, or clear the selection for a whole-record question, and open the Open questions section), the row's verb and the assistant door; a ringed count chip on the subject it is open for, behind the badges toggle, a folded box wearing it for a hidden member; and a "next" tag on the row in the Open questions section. Absent `next` means no next question: the tab is not offered and a remembered tab falls back to the query.
